...
export function BillingContent() {
  ...
  if (error) {
    ...
    return (
      <div>
        An error occurred while fetching billing data. Please try again later.
      </div>
    );
  }
  ...
  return (
    <>
      <Alert variant="destructive" className="mb-6">
        ...
        <AlertDescription>
          We have identified an issue resulting in some delayed events related
          to billing for requests to the Anthropic API and Console. This issue
          does not affect success rates on the API, but will result in delayed
          visibility on usage in the Console. All usage will still be billed as
          expected.
        </AlertDescription>
      </Alert>
      <h2 className="text-xl font-bold mb-4">Credit balance</h2>
      <p className="mb-4">
        Your credit balance will be consumed with API and Workbench usage. You
        can add funds directly or set up auto-reload thresholds.
      </p>
      <Card className="mb-6">
        <CardContent className="flex flex-col md:flex-row justify-between items-center p-6">
          <div>
            <h3 className="text-2xl font-bold">
              US$ {billingData.creditBalance.toFixed(2)}
            </h3>
            <p className="text-sm text-gray-500">Remaining Balance</p>
          </div>
          <div className="mt-4 md:mt-0">
            <p className="mb-2">
              Charged to Mastercard •••• {billingData.cardLastFour}
            </p>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div className="flex items-center space-x-2 mb-2 md:mb-0">
          ...
          <span>
            Auto reload is{" "}
            {billingData.autoReloadEnabled ? "enabled" : "disabled"}.
          </span>
        </div>
        <Button
          variant="outline"
          onClick={handleEditAutoReload}
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <p>
            Contact the Anthropic accounts team to switch to monthly invoicing.
          </p>
          <Button
            className="mt-2"
